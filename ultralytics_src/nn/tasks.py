...
from ultralytics.nn.modules.CMIFE import CMIFE
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m is CMIFE:
            # CMIFE模块处理 - 支持多输入
            if verbose:
                LOGGER.info(f"Processing CMIFE: from={f}, args={args}")

            # 检查args是否为空
            if not args:
                # 如果args为空，设置默认输出通道
                if verbose:
                    LOGGER.warning(f"CMIFE args is empty, using default channels")
                out_channels = 256  # 默认值，可以根据需要调整
                if out_channels != nc:
            ...
            if isinstance(f, list):
                ...
                for idx in f:
                    if idx == -1:
                        input_channels.append(ch[-1])
                    elif isinstance(idx, int) and idx < len(ch):
                        input_channels.append(ch[idx])
                    else:
                        input_channels.append(ch[-1])

                # 重新组织参数
                if args:
                    args = [input_channels, out_channels] + args[1:]
                else:
                    args = [input_channels, out_channels]
                c2 = out_channels
            else:
                # 单输入情况
                c1 = ch[f] if isinstance(f, int) and f < len(ch) else ch[-1]
                if args:
            ...
            # CMIFES模块处理 - 与CMIFE相同逻辑，SE轻量化版本
            if verbose:
                LOGGER.info(f"Processing CMIFES: from={f}, args={args}")
            if not args:
                out_channels = 256
                if out_channels != nc:
                    out_channels = make_divisible(min(out_channels, max_channels) * width, 8)
        ...
        try:
            ...
            print(f"Error creating module {m} with args {args}: {e}")
            raise
